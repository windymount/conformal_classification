...
parser.add_argument('--batch-size', metavar='BSZ', type=int, help='batch size', default=128)
parser.add_argument('--arch', type=str, default="ResNet50")
parser.add_argument('--num-workers', metavar='NW', help='number of workers', default=0)
parser.add_argument('--num-calib', metavar='NCALIB', type=int, help='number of calibration points', default=10000)
parser.add_argument('--num-test', type=int, help='number of test points', default=5000)
...
parser.add_argument('--use-rs-model', action='store_true', default=False)
parser.add_argument('--rs-model', type=str)
...
if __name__ == "__main__":
    ...
    # Normalization is seperated for adv reasons.
    ...
    # Initialize loaders 
    val_data_subset, _ = torch.utils.data.random_split(imagenet_val_data, [args.num_test, len(imagenet_val_data) - args.num_test])
    calib_loader = torch.utils.data.DataLoader(imagenet_calib_data, batch_size=args.batch_size, shuffle=True, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(val_data_subset, batch_size=args.batch_size, shuffle=True, pin_memory=True)

    cudnn.benchmark = True
    # PGD attack images
    def get_adv_loaders(model):
        adv = torchattacks.PGD(model, eps=2/255)
        def adv_collate(batch, bs=8):
            imgs, targets = zip(*batch)
            adv_img = []
            for i in range(0, len(imgs), bs):
                imgs_bc = torch.stack(imgs[i:i+bs])
                targets_bc = torch.Tensor(targets[i:i+bs]).cuda().type(torch.long)
                adv_img.append(adv(imgs_bc, targets_bc))
            return torch.cat(adv_img), torch.Tensor(targets).cuda().type(torch.long)

        adv_loader = torch.utils.data.DataLoader(val_data_subset, batch_size=args.batch_size, shuffle=True, collate_fn=adv_collate)
        adv_calib_loader = torch.utils.data.DataLoader(imagenet_calib_data, batch_size=args.batch_size, shuffle=True, collate_fn=adv_collate)
        return adv_loader, adv_calib_loader
    # Sanity check
    def evaluate(model, dataloader):
        num_imgs, correct = 0, 0
        for img, label in tqdm(dataloader):
            img, label = normalization(img.cuda()), label.cuda()
            with torch.no_grad():
    ...
    if args.exp2:
        ...
        adv_model = nn.Sequential(normalization, adv_model)
        adv_val_loader, adv_calib_loader = get_adv_loaders(adv_model)
        ...
        validate(val_loader, adv_val_loader, cmodel_adv)
    if args.exp3:
        # Can RS model be applied?
        model = utils.get_model(args.arch)
        if args.use_rs_model:
            model.load_state_dict(torch.load(args.rs_model)['state_dict'])
        model.eval()
        model = nn.Sequential(normalization, model)
        adv_val_loader, adv_calib_loader = get_adv_loaders(model)
        smooth_model = Smooth(model, 1000, args.sigma, torch.device('cuda'), 
                              mode=args.mode)
        cmodel_clean = ConformalModel(smooth_model, calib_loader, alpha=0.1, lamda_criterion='size', prob_model=True)
        print("Validate clean data model.")
        validate(val_loader, adv_val_loader, cmodel_clean)
        cmodel_adv = ConformalModel(smooth_model, adv_calib_loader, alpha=0.1, lamda_criterion='size', prob_model=True)
        print("Validate adv data model.")
        validate(val_loader, adv_val_loader, cmodel_adv)