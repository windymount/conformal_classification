  ...
  def _sample_noise(self, x: torch.tensor, num: int, batch_size) -> np.ndarray:
    with torch.no_grad():
      ...
      for _ in range(ceil(num / batch_size)):
        ...
        if self.mode == 'logit':
          p_soft = predictions.sum(0)
          result_soft += p_soft.cpu().numpy()
        predictions *= self.beta
        if self.mode == 'hard' or self.mode == 'both' or self.mode == 'flex':
      ...
      if self.mode == 'hard' or self.mode == 'flex':
        return result_hard
      if self.mode == 'logit':
        return result_soft / num
      if self.mode == 'soft':
        return result_soft
      else:
